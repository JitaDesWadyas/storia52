...
(() => {
  ...
  const namesMarkup = session => `<section class="setup-panel"><div class="setup-head"><div><p class="eyebrow">GIOCATORI</p><b>Nomi e numero partecipanti</b></div><select data-player-count>${Array.from({ length: 9 }, (_, index) => index + 2).map(value => `<option value="${value}"${value === session.count ? ' selected' : ''}>${value} giocatori</option>`).join('')}</select></div><div class="name-grid">${Array.from({ length: session.count }, (_, index) => `<label class="field"><span>Giocatore ${index + 1}</span><input data-player-name="${index}" value="${S.esc(session.names[index] || '')}" placeholder="Nome facoltativo"></label>`).join('')}</div></section>`;
  ...
  S.renderSetup = (mode, existing = null) => {
    ...
    S.play.querySelector('[data-player-count]').addEventListener('change', event => {
      ...
      S.renderSetup(mode, session);
    });
    S.play.querySelectorAll('[data-player-name]').forEach(input => input.addEventListener('input', sync));
    S.play.querySelectorAll('[data-source]').forEach(button => button.addEventListener('click', () => { sync(); session.source = button.dataset.source; S.renderSetup(mode, session); }));
    S.play.querySelectorAll('[data-delivery]').forEach(button => button.addEventListener('click', () => { sync(); session.delivery = button.dataset.delivery; S.renderSetup(mode, session); }));
