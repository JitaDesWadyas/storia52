...
(() => {
  ...
  S.renderObjectives = session => {
    ...
    S.play.querySelector('[data-objectives-done]').addEventListener('click', () => S.renderPreparation(session));
  };
  ...
  S.openObjective = (session, index, duringGame) => {
    ...
    const draw = () => {
      ...
      card.querySelector('[data-toggle-objective]').addEventListener('click', () => { revealed = !revealed; if (revealed) seen = true; draw(); });
