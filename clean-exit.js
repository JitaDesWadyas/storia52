...
(() => {
  ...
  S.openExitModal = () => {
    ...
    host.querySelector('[data-exit-finished]')?.addEventListener('click', () => { close(); S.renderFinished(); });
    host.querySelector('[data-exit-abandon]').addEventListener('click', () => { close(); S.clear(); S.renderHome(); });
  };
  ...
  S.renderFinished = () => {
    ...
    S.play.querySelector('[data-finished-new]').addEventListener('click', S.renderHome);
  };
