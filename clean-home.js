...
(() => {
  ...
  const tutorialSteps = [
    {
      badge: 'INIZIO', player: 'Tre giocatori', card: '?', label: 'Obiettivi segreti',
      title: 'Una storia. Tre finali diversi.',
      instruction: 'Si gioca a turno, una persona alla volta. Marta, Luca e Sara ricevono un obiettivo che gli altri non vedono.',
      addition: '', objective: '', objectiveOwner: '', tone: 'neutral'
    },
    {
      badge: 'TURNO 1', player: 'Marta', card: '♣ 8', label: 'Azione',
      title: 'Marta fa agire un personaggio.', instruction: 'Il padrone chiude le porte e ritira i telefoni.',
      addition: 'Il padrone chiude le porte e ritira i telefoni.', objective: 'Fai ricadere la colpa sul padrone.', objectiveOwner: 'Marta', tone: 'clubs'
    },
    {
      badge: 'TURNO 2', player: 'Luca', card: '♦ J', label: 'Scoperta',
      title: 'Luca aggiunge un indizio.', instruction: 'Una foto bruciata collega il quadro alla villa.',
      addition: 'Una foto bruciata collega il quadro alla villa.', objective: 'Rivela un segreto di famiglia.', objectiveOwner: 'Luca', tone: 'diamonds'
    },
    {
      badge: 'TURNO 3', player: 'Sara', card: '♠ 5', label: 'Ostacolo',
      title: 'Sara rende tutto più difficile.', instruction: 'Qualcuno blocca la porta e riaccende il camino.',
      addition: 'Qualcuno blocca la porta e riaccende il camino.', objective: 'Impedisci al gruppo di uscire.', objectiveOwner: 'Sara', tone: 'spades'
    },
    {
      badge: 'FINALE', player: 'Marta', card: '♥ 10', label: 'Ultima carta',
      title: 'Marta collega tutto e prova a chiudere.', instruction: tutorialFinale,
      addition: tutorialFinale, objective: 'Fai ricadere la colpa sul padrone.', objectiveOwner: 'Marta', tone: 'reveal'
    }
  ];
  ...
  const storyMarkup = index => {
    ...
    return `<section class="tutorial-storyboard" aria-label="Storia costruita durante la partita">
      ...
      ${showFinale ? `<div class="tutorial-final-line"><span>FINALE</span><p>${S.esc(tutorialFinale)}</p></div>` : ''}
    </section>`;
  };
    ...
    return `<div class="tutorial-experience tutorial-tone-${step.tone}" data-current-step="${index}">
      ...
      <div class="tutorial-frame">
        ...
        <section class="tutorial-stage">
          <aside class="tutorial-play-card" aria-label="Carta: ${S.esc(step.card)}"><span class="tutorial-card-corner">${S.esc(step.card)}</span><span class="tutorial-card-suit">${S.esc(step.card.split(' ')[0])}</span><strong>${S.esc(step.label)}</strong></aside>
          <div class="tutorial-copy"><h3>${S.esc(step.title)}</h3><p>${S.esc(step.instruction)}</p>${!isReveal && step.objective ? `<div class="tutorial-secret"><span>OBIETTIVO DI ${S.esc(step.objectiveOwner).toUpperCase()}</span><p>${S.esc(step.objective)}</p></div>` : ''}${isReveal ? revealMarkup() : ''}</div>
        </section>
      </div>
    </div>`;
  };
  ...
  const infoMarkup = () => `<div class="screen-heading modal-heading creator-heading"><p class="eyebrow">DIETRO E POI?</p><h2>Da un mazzo normale a una storia contesa.</h2><p><strong>E POI?</strong> nasce per rendere il gioco narrativo immediato, sociale e comprensibile anche a chi non frequenta giochi di ruolo o regolamenti pesanti.</p></div>
    <div class="creator-page"><article class="creator-profile"><img src="${CREATOR_PHOTO}" alt="Jita DesWadyas"><div><p class="eyebrow">CHI SONO</p><h3>Jita DesWadyas</h3><span>JitaDiSwadya · Italia</span><p>Sono uno sviluppatore, game designer, worldbuilder e autore indipendente. Creo giochi, strumenti web e mondi narrativi; con il progetto musicale <strong>JitaDiSwadya</strong> trasformo storie e sensazioni in canzoni.</p></div></article>
    <section class="creator-origin"><div><p class="eyebrow">COME È NATO</p><h3>Il problema non era inventare una storia.</h3></div><div class="creator-origin-copy"><p>Volevo un gioco in cui tutti costruissero <strong>la stessa storia</strong>, ma senza collaborare davvero allo stesso finale. Il mazzo da 52 carte era già perfetto: accessibile, economico e presente in quasi ogni casa.</p><p>I semi sono diventati tipi di scena, mentre gli <strong>obiettivi segreti</strong> hanno aggiunto il conflitto. Da lì è nata la web app: prima per provare il ritmo, poi per raccogliere storie, spiegare le regole e preparare una futura versione fisica.</p></div></section>
    <section class="creator-project-section"><div class="creator-section-heading"><p class="eyebrow">ALTRI PROGETTI</p><h3>Lo stesso filo, in forme diverse.</h3></div><div class="creator-project-grid"><article><b>FEIM</b><p>Un MMORPG sandbox 2D con mondo persistente, economia dei giocatori e narrazione dinamica.</p></article><article><b>FEIM: On The Table</b><p>Una piattaforma per campagne da tavolo, mappe, combattimenti e gestione del mondo.</p></article><article><b>feimWorlds</b><p>Un motore 2D costruito in Go ed Ebitengine per creare mondi e giochi modulari.</p></article><article><b>FEIM Notes</b><p>Uno spazio visuale per note, collegamenti e worldbuilding.</p></article><article><b>JitaDiSwadya</b><p>Musica narrativa: storie, scene e sensazioni trasformate in canzoni.</p></article></div></section>
    <section class="creator-direction"><p class="eyebrow">DOVE VUOLE ARRIVARE</p><h3>Prima deve funzionare bene al tavolo.</h3><p>La priorità è rendere ogni partita chiara, veloce e divertente. Poi arriveranno più storie, una modalità multi-telefono più solida e, se il gioco dimostra di reggere, un mazzo personalizzato con scatola e regolamento.</p><div class="creator-path"><span class="done">Idea</span><span class="done">Web app</span><span class="current">Test</span><span>Più storie</span><span>Mazzo fisico</span></div></section></div>`;

  S.homeMarkup = () => {
    const saved = S.load();
    return `<section class="surface hero product-hero"><div class="hero-copy"><div class="hero-kicker"><p class="eyebrow">GIOCO NARRATIVO · CARTE · OBIETTIVI SEGRETI</p></div><h2 class="hero-title"><span class="line">Un gioco che mette alla prova</span><span class="line line-accent">creatività e immaginazione,</span><span class="line line-final">improvvisando.</span></h2><p class="hero-intro">Aggiungi una scena seguendo le carte. Mentre la storia prende forma, prova a guidarla verso il tuo <strong>obiettivo segreto</strong> senza farti scoprire.</p><div class="hero-divider" aria-hidden="true"><span>Una storia · più piani</span></div></div><div class="hero-actions"><button type="button" class="primary" data-home-play><span class="button-icon" aria-hidden="true">?</span><span>Gioca ora</span><span class="button-arrow" aria-hidden="true">→</span></button><button type="button" class="secondary" data-open-panel="tutorial"><span class="button-icon" aria-hidden="true">▶</span><span>Guarda come funziona</span><span class="button-arrow" aria-hidden="true">↗</span></button></div></section><div class="home-divider" aria-hidden="true"><span>✦</span></div><div class="home-grid product-menu compact-product-menu">${saved ? `<button type="button" class="resume-card" data-home-resume><span class="index">↻</span><span><b>Riprendi la partita</b><small>${S.esc(S.sourceLabel(saved))} · ${saved.count || 0} giocatori</small></span><i>→</i></button>` : ''}<button type="button" class="choice-card" data-open-panel="how"><span class="index">?</span><span><b>Inizia da qui</b><small>Il gioco spiegato in 2 minuti.</small></span><i>→</i></button><button type="button" class="choice-card" data-open-panel="rules"><span class="index">📖</span><span><b>Le regole complete</b><small>Tutte le meccaniche, quando ti servono.</small></span><i>→</i></button><button type="button" class="choice-card creator-menu-card" data-open-panel="info"><span class="creator-menu-image" aria-hidden="true"><img src="storia52-cards-logo.svg" alt=""></span><span><b>Dietro E POI?</b><small>La nascita del gioco, il creatore e gli altri progetti.</small></span><i>→</i></button></div>`;
  };
