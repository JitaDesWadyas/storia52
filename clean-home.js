...
(() => {
  ...
  const tutorialSteps = [
    ...
    { kind: 'ready', eyebrow: '10 · ADESSO PUOI GIOCARE', title: 'Questo è l’intero ciclo.', body: 'Cambia una carta → gioca una carta → racconta una scena → pesca se vuoi → passa il turno.' }
  ];
  ...
  const exampleMarkup = sceneIndex => {
    ...
    return `<div class="tutorial-example tutorial-tone-${scene.tone}"><p class="tutorial-continuity-note"><strong>Stessa partita:</strong> questa scena continua tutti i fatti mostrati prima.</p><div class="tutorial-example-main"><div class="tutorial-card-demo"><small>CARTA GIOCATA</small><b>${S.esc(scene.card)}</b><span>${S.esc(scene.meaning)}</span></div><div class="tutorial-decision"><div><span>1. LEGGE L’EFFETTO</span><p>Con <strong>${S.esc(scene.card)}</strong> deve aggiungere: <strong>${S.esc(scene.meaning.toLowerCase())}</strong>.</p></div><div><span>2. PENSA AL SUO OBIETTIVO</span><p>${S.esc(objectives[scene.player])}</p></div><div class="tutorial-spoken"><span>3. DICE UNA SOLA SCENA</span><blockquote>“${S.esc(scene.said)}”</blockquote></div><div><span>4. PERCHÉ FUNZIONA</span><p>${S.esc(scene.why)}</p></div></div></div>${storyTimeline(sceneIndex + 1)}<aside class="tutorial-pass"><b>Turno concluso.</b><span>Dopo la scena può pescare una carta. Poi continua la persona successiva dalla storia completa qui sopra.</span></aside></div>`;
  };
  ...
  const stepContent = step => {
    ...
    return `<div class="tutorial-cycle"><span>Cambia 1 carta</span><i>→</i><span>Gioca 1 carta</span><i>→</i><span>Racconta</span><i>→</i><span>Pesca se vuoi</span><i>→</i><span>Passa</span></div><p class="tutorial-cycle-note">Il cambio è obbligatorio con 2 o più carte; con una sola carta è facoltativo.</p><div class="tutorial-ready-actions"><button type="button" class="primary" data-tutorial-play>Prepara una partita <span aria-hidden="true">→</span></button><button type="button" class="secondary" data-tutorial-rules>Consulta le regole</button></div>`;
  };
