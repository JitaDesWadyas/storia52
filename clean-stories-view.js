...
(() => {
  ...
  S.renderStories = (session, scroll = true) => {
    ...
    S.play.querySelector('[data-page-prev]')?.addEventListener('click', () => { S.storyUi.page -= 1; S.renderStories(session, false); });
    S.play.querySelector('[data-page-next]')?.addEventListener('click', () => { S.storyUi.page += 1; S.renderStories(session, false); });
    S.play.querySelector('[data-collection-info]')?.addEventListener('click', event => S.openCollectionInfo(event.currentTarget.dataset.collectionInfo));
