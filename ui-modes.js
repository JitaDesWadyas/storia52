...
function quickMode(game){
  const state={mode:'random',cards:{protagonist:randomCard(),situation:randomCard(),problem:randomCard(),objective:randomCard()}};
  function setup(){
    game.innerHTML=`<p class="eyebrow">GENERATORE RAPIDO</p><h2>Come vuoi creare la storia?</h2>${choiceToggle(state.mode)}${state.mode==='manual'?`<div class="card-choice-list">${selectorHtml('protagonist','Protagonista','protagonist',state.cards.protagonist)}${selectorHtml('situation','Situazione','situation',state.cards.situation)}${selectorHtml('problem','Problema','problem',state.cards.problem)}${selectorHtml('objective','Obiettivo segreto','objective',state.cards.objective)}</div>`:'<p class="setup-note">Il sistema pescherà protagonista, situazione, problema e obiettivo.</p>'}<div class="button-row"><button type="button" id="makeQuick" class="button button-primary">Crea storia</button></div>`;
    ...
    $('#makeQuick').addEventListener('click',()=>{
      const seed=createCode();const story=state.mode==='manual'?{protagonist:state.cards.protagonist,situation:state.cards.situation,problem:state.cards.problem}:storyFromSeed(seed);
...
function quickResult(game,seed,story,objective){
  let open=false;
  function draw(){
    game.innerHTML=`<div class="game-header"><div><p class="eyebrow">GENERATORE RAPIDO</p><h2>Il vostro incipit</h2></div><span class="game-code">${seed}</span></div>${storyStack(story)}<h3>Obiettivo del giocatore</h3><div class="secret-card${open?' open':''}">${open?secretContent(objective):secretClosed()}</div><div class="button-row"><button type="button" id="quickReveal" class="button button-primary">${open?'Nascondi':'Rivela obiettivo'}</button><button type="button" id="quickAgain" class="button button-dark">Nuova storia</button></div>`;
...
function singleMode(game){
  ...
  const state={count:4,seed:createCode(),mode:'random',cards:{protagonist:randomCard(),situation:randomCard(),problem:randomCard()},objectives:Array.from({length:10},()=>randomCard())};
  function setup(){
    ...
    if(state.mode==='manual'){
      ...
      bindSelectors(game,map,()=>{
        state.cards.protagonist=map.protagonist;state.cards.situation=map.situation;state.cards.problem=map.problem;
...
function makeInviteUrl(host,player,objective){
  const url=new URL(location.href);url.search='';url.hash='play';url.searchParams.set('room',host.seed);url.searchParams.set('player',String(player));url.searchParams.set('story',serializeStory(host.story));url.searchParams.set('objective',serializeCard(objective));return url.toString();
}
...
function showMulti(game,invite){
  let open=false;
  function draw(){
    game.innerHTML=`<div class="game-header"><div><p class="eyebrow">GIOCATORE ${invite.player}</p><h2>Incipit pubblico</h2></div><span class="game-code">${invite.seed}</span></div>${storyStack(invite.story)}<h3>Il tuo Obiettivo segreto</h3><div class="secret-card${open?' open':''}">${open?secretContent(invite.objective):secretClosed()}</div><div class="button-row"><button type="button" id="multiReveal" class="button button-primary">${open?'Nascondi':'Rivela'}</button><button type="button" id="leaveInvite" class="button button-ghost">Esci dall’invito</button></div>`;
